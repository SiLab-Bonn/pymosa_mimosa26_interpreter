        ...
        for curr_plane_id in active_m26_planes:
            ...
            while index >= 0:
                if telescope_data[index]['plane'] == curr_plane_id:
                    if telescope_data[index]['frame_id'] > last_completed_m26_frame_ids[curr_plane_id]:
                        telescope_data[index]['frame_status'] |= DATA_ERROR
                    else:
                        break
                index -= 1
...
def _build_events(trigger_data, trigger_data_index, telescope_data, telescope_data_index, hits, hits_index, last_completed_m26_frame_ids, timing_offset, build_all_events, active_m26_planes):
    ...
    curr_event_status = np.zeros(shape=6, dtype=np.uint32)
        ...
        while curr_telescope_data_index <= telescope_data_index:
            ...
            if not finished_event[curr_plane] and (build_all_events or curr_frame_id <= last_completed_m26_frame_ids[curr_plane]):
                ...
                if hit_timestamp_start <= trigger_timestamp and trigger_timestamp < hit_timestamp_stop:
                    ...
                    curr_event_status[curr_plane] |= telescope_data[curr_telescope_data_index]['frame_status'] | trigger_status
                    ...
                    if np.all(finished_event):
                        ...
                        while trigger_event_number == hits[index]['event_number']:
                            hits[index]['event_status'] = curr_event_status[hits[index]['plane']]
                            index -= 1
                        break
