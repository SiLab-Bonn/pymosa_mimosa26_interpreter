...
def analyze_raw_data(raw_data_file, time_reference_file, trigger_data_format):  # Mimosa26 raw data analysis
...
def process_dut(raw_data_file, time_reference_file, trigger_data_format, transpose=False, frame=False):
    ''' Process raw data files
        ...
        trigger_data_format : uint
            ...
            Only trigger data format 2 is supported, since the event building requires a trigger timestamp in order to work reliably.
        transpose : boolean
            If True, switch column and row index. Default is False
        frame : boolean
            If True, store Mimosa26 frame ID. Default is False
    '''
    # analyze raw data
    ...
    for plane in range(1, 7):
        logging.info("Formating hit table of plane %i...", plane)
